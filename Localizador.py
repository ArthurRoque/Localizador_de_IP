...
class Finder():
        ...
        if response.status_code != 200:
            ...
            try:
                response = {'city': response['city'],
                            'country': PonsTranslator(source='english', target='portuguese').translate(response['country'], return_all=False),
        ...
        if self.IP != '':
            ...
            try:
                response = {'city': response['geoplugin_city'],
                            'country': PonsTranslator(source='english', target='portuguese').translate(response['geoplugin_countryName']),
    ...
    def ip_api(self):
        ...
        if response.status_code != 200:
            ...
            try:
                response = {'city': response['city'],
                            'country': PonsTranslator(source='english', target='portuguese').translate(response['country']),
                            'latitude': response['lat'],
                            'longitude': response['lon']}
    ...
    def HG_geoip(self):
        ...
        if response.status_code != 200:
            ...
            try:
                response = {'city': response['city'],
                            'country': PonsTranslator(source='english', target='portuguese').translate(response['country_name']),
                            'latitude': response['latitude'],
                            'longitude': response['longitude']}
    ...
    def get_location(self):
        ...
        if self.latitude != [] and self.valid_IP != False:
            ...
            self.latitude_reliability = f'{round(latitude_for_comparation.count(latitude_mode[0:6])/len(latitude_for_comparation), 4) * 100}%'
        ...
        if self.longitude != [] and self.valid_IP != False:
            ...
            self.longitude_reliability = f'{round(longitude_for_comparation.count(longitude_mode[0:6])/len(longitude_for_comparation), 4) * 100}%'
        ...
        if self.city != [] and self.valid_IP != False:
            ...
            self.city_reability = f'{round(city_without_accents.count(city_mode_without_accents)/len(city_without_accents),4) * 100}%'
            self.city = city_mode
